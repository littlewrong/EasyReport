...
export function useTabbar() {
  ...
  const pruneStaleTabs = () => {
    ...
    const routeNames = new Set(routes.map((item) => item.name).filter(Boolean));
    ...
    tabbarStore.tabs = tabbarStore.tabs.filter((tab) => {
      const legacyAnalytics =
        tab.name === 'Analytics' ||
        tab.path === '/analytics' ||
        tab.fullPath === '/analytics';
      const routeExists = tab.name
        ? routeNames.has(tab.name)
        : routePaths.has(tab.path);
      const keep = routeExists && !legacyAnalytics;
      if (!keep) removedKeys.push((tab.key as string) || getTabKey(tab));
      return keep;
    });

    if (removedKeys.length > 0) {
      tabbarStore.visitHistory.remove(...removedKeys);
