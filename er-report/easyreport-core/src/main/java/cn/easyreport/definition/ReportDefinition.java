...
import cn.easyreport.definition.searchform.RenderContext;
...
public class ReportDefinition implements Serializable{
	...
	private String buildStyle(){
		// System.out.println(">>>>> buildStyle() 开始执行 <<<<<");
	...
	private int getColumnWidth(int columnNumber,int colSpan){
		// System.out.println("@@@@@ getColumnWidth: columnNumber="+columnNumber+", colSpan="+colSpan);
		int width=0;
		if(colSpan>0)colSpan--;
		int start=columnNumber,end=start+colSpan;
		// System.out.println("  范围: start="+start+", end="+end);
		for(int i=start;i<=end;i++){
			for(ColumnDefinition col:columns){
				if(col.getColumnNumber()==i){
					// System.out.println("  列"+i+": width="+col.getWidth()+" (跳过="+(col.getWidth()<=0)+")");
					// 跳过隐藏的列(width为0的列)
					if(col.getWidth()>0){
						width+=col.getWidth();
					}
		...
		// System.out.println("  最终width="+width);
		return width;
	}
