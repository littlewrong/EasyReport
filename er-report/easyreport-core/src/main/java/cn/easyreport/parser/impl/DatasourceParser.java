...
import cn.easyreport.definition.dataset.BeanDatasetDefinition;
...
import cn.easyreport.definition.dataset.Parameter;
import cn.easyreport.definition.dataset.SqlDatasetDefinition;
...
public class DatasourceParser implements Parser<DatasourceDefinition> {
	...
	private List<DatasetDefinition> parseDatasets(Element element){
		...
		for(Object obj:element.elements()){
			...
			if(type.equals("sql")){
				...
				dataset.setName(ele.attributeValue("name"));
				dataset.setUrl(ele.attributeValue("url"));
				dataset.setMethod(ele.attributeValue("method"));
				dataset.setRequestBody(parseRequestBody(ele));
				dataset.setDataPath(ele.attributeValue("dataPath"));
				dataset.setFields(parseFields(ele));
				dataset.setParameters(parseParameters(ele));
				list.add(dataset);
			}
	...
	private String parseRequestBody(Element element){
		// 首先尝试从子元素<requestBody>中读取（CDATA格式）
		for(Object obj:element.elements()){
			if(obj==null || !(obj instanceof Element)){
				continue;
			}
			Element ele=(Element)obj;
			if(ele.getName().equals("requestBody")){
				return ele.getText();
			}
		}
		// 兼容旧格式：如果没有子元素，尝试从属性中读取
		String requestBodyAttr = element.attributeValue("requestBody");
		return requestBodyAttr != null ? requestBodyAttr : "";
	}
}
