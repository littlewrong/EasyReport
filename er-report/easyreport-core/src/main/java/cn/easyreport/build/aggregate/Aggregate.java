...
public abstract class Aggregate {
	...
	protected String resolveProperty(DatasetExpression expr, Context context){
		if(!expr.isUseIndex()){
			return expr.getProperty();
		}
		Integer idx=expr.getIndex();
		if(idx==null){
			return null;
		}
		List<String> fields=context.getDatasetFields(expr.getDatasetName());
	...
	protected Object fetchValueByProperty(Object obj, String prop){
		if(prop==null){
			return null;
		}
		return Utils.getProperty(obj, prop);
	}
	
	protected Object fetchValue(DatasetExpression expr, Context context, Object obj){
		String prop=resolveProperty(expr, context);
		return fetchValueByProperty(obj, prop);
	}
