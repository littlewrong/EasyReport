...
import cn.easyreport.build.cell.CellBuilder;
import cn.easyreport.build.cell.NoneExpandBuilder;
import cn.easyreport.build.cell.down.DownExpandBuilder;
import cn.easyreport.build.cell.right.RightExpandBuilder;
import cn.easyreport.build.paging.BasePagination;
import cn.easyreport.build.paging.Page;
import cn.easyreport.build.paging.PagingBuilder;
import cn.easyreport.definition.Band;
import cn.easyreport.definition.Expand;
...
import cn.easyreport.definition.datasource.BuildinDatasourceDefinition;
import cn.easyreport.definition.datasource.DatasourceDefinition;
import cn.easyreport.definition.datasource.DatasourceProvider;
import cn.easyreport.definition.datasource.HttpDatasourceDefinition;
import cn.easyreport.definition.datasource.JdbcDatasourceDefinition;
import cn.easyreport.definition.datasource.SpringBeanDatasourceDefinition;
...
public class ReportBuilder extends BasePagination implements ApplicationContextAware{
	public static final String BEAN_ID="easyreport.reportBuilder";
	...
	public Report buildReport(ReportDefinition reportDefinition,Map<String,Object> parameters) {
		...
		// 将Paper配置存入Request Attribute，供HTTP数据集使用
		try {
			org.springframework.web.context.request.ServletRequestAttributes attrs =
				(org.springframework.web.context.request.ServletRequestAttributes)
				org.springframework.web.context.request.RequestContextHolder.getRequestAttributes();
			if (attrs != null && reportDefinition.getPaper() != null) {
				attrs.getRequest().setAttribute("__easyreport_paper", reportDefinition.getPaper());
			}
		} catch (Exception e) {
			// 忽略异常，不影响主流程
		}
	...
	private void recomputeCells(Report report,Context context){
		...
		if(pagingMode.equals(PagingMode.fitpage)){
			...
			for(int i=0;i<rowSize;i++){
				...
				if(!shouldBreak && row.isPageBreak()){
					shouldBreak=true;
				}
				if(shouldBreak){
					Page newPage=buildPage(pageRows,pageRepeatHeaders,pageRepeatFooters,titleRows,pageIndex,report);
