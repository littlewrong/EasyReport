...
import cn.easyreport.export.excel.high.CellStyleContext;
...
public class ExcelBuilderDirect extends ExcelBuilder {
	public void build(Report report, OutputStream outputStream) {
		...
		// 注意：由于报表可能有合并单元格需要回溯访问之前的行，
		// 如果使用内存窗口会导致"Attempting to write a row that is already written to disk"错误
		// 因此这里使用-1（不限制内存窗口），所有行保留在内存中
		// 对于超大数据量（10万+），请使用流式导出功能
		SXSSFWorkbook wb = new SXSSFWorkbook(-1);
		CreationHelper creationHelper=wb.getCreationHelper();
		Paper paper=report.getPaper();
		try{
			List<Column> columns=report.getColumns();
			Map<Row,Map<Column,cn.easyreport.model.Cell>> cellMap=report.getRowColCellMap();
			...
			for(Row r:rows){
	        	...
	        	Map<Column,cn.easyreport.model.Cell> colCell=cellMap.get(r);
	        	int skipCol=0;
	        	for(int i=0;i<columnSize;i++){
	        		...
	        		cn.easyreport.model.Cell cellInfo=null;
	        		if(colCell!=null){
