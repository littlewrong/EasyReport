 ...
 * the License.
 ******************************************************************************/
package cn.easyreport.export.word.high;
...
import cn.easyreport.definition.BorderStyle;
...
import cn.easyreport.export.Producer;
import cn.easyreport.export.word.DxaUtils;
...
import cn.easyreport.utils.ImageUtils;
import cn.easyreport.utils.UnitUtils;
...
public class WordProducer implements Producer{
	public static final String BEAN_ID="easyreport.wordExporter";
			...
			if(StringUtils.isNotBlank(base64Data)){
				...
				try{
					...
					if(imageType.equals("jpeg")){
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_JPEG, "easyreport-"+rowNumber+"-"+columnNumber+".jpg", Units.toEMU(width), Units.toEMU(height));					
					}else if(imageType.equals("png")){
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_PNG, "easyreport-"+rowNumber+"-"+columnNumber+".png", Units.toEMU(width), Units.toEMU(height));					
					}else if(imageType.equals("gif")){
						run.addPicture(inputStream, XWPFDocument.PICTURE_TYPE_GIF, "easyreport-"+rowNumber+"-"+columnNumber+".gif", Units.toEMU(width), Units.toEMU(height));					
					}
				}catch(Exception ex){
					throw new ReportComputeException(ex);
				}finally{
