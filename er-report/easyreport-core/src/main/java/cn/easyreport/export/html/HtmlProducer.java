 ...
 * the License.
 ******************************************************************************/
package cn.easyreport.export.html;
...
import cn.easyreport.definition.Alignment;
import cn.easyreport.definition.Border;
import cn.easyreport.definition.CellStyle;
...
import cn.easyreport.model.Column;
import cn.easyreport.model.Image;
import cn.easyreport.model.Report;
import cn.easyreport.model.Row;
...
public class HtmlProducer{
	public String produce(Report report) {
		// System.out.println(">>>>> HtmlProducer.produce(Report) 被调用了! <<<<<");
		List<Row> rows=report.getRows();
		List<Column> columns=report.getColumns();
		// System.out.println("行数: "+rows.size()+", 列数: "+columns.size());
	...
	private StringBuilder buildTable(Context context,List<Row> rows, List<Column> columns,Map<Row, Map<Column, Cell>> cellMap,boolean breakPage,boolean forPage) {
		...
		for(int i=0;i<rowSize;i++){
			...
			for(int j=0;j<colSize;j++){
				...
				// System.out.println("@@@ 调用buildCustomStyle for cell: "+cell.getName()+", colspan="+cell.getColSpan());
				String style=buildCustomStyle(cell,columns);
				// System.out.println("@@@ buildCustomStyle返回值: ["+style+"]");
				sb.append(" "+style+"");
				sb.append(">");
	...
	private int buildWidth(List<Column> columns,int colIndex,int colSpan){
		...
		// System.out.println("===== buildWidth调用 =====");
		// System.out.println("colIndex="+colIndex+", colSpan="+colSpan+", start="+start+", end="+end);
		for(int i=start;i<end;i++){
			Column col=columns.get(i);
			// System.out.println("  列"+i+": width="+col.getWidth()+", isHide="+col.isHide());
			// 跳过隐藏列,避免累加隐藏列宽度导致合并列宽度错误
			if(col.getWidth()>0 && !col.isHide()){
				width+=col.getWidth();
				// System.out.println("    累加宽度: "+col.getWidth()+" -> 总宽度: "+width);
			}else{
				// System.out.println("    跳过隐藏列");
			}
		}
		// System.out.println("最终宽度: "+width);
		// System.out.println("========================");
		return width;
	}
	...
	private String buildCustomStyle(Cell cell,List<Column> columns){
		...
		// 始终输出运行时宽度以覆盖 CSS 类中的静态宽度
		int colSpan=cell.getColSpan();
		int colWidth=0;
		// System.out.println("##### buildCustomStyle 单元格: "+cell.getName()+" #####");
		// System.out.println("colSpan="+colSpan+", columnNumber="+cell.getColumn().getColumnNumber());
		if(colSpan>0){
			// 合并列:累加所有列宽,跳过隐藏列
			int colNumber=cell.getColumn().getColumnNumber()-1;
			// System.out.println("合并列: 调用buildWidth(columns, "+colNumber+", "+colSpan+")");
			colWidth=buildWidth(columns,colNumber,colSpan);
			// System.out.println("buildWidth返回: "+colWidth);
		}else{
			// 单列:直接使用列宽
			colWidth=cell.getColumn().getWidth();
			// System.out.println("单列宽度: "+colWidth);
		}
		// System.out.println("最终设置width="+colWidth+"pt");
		// System.out.println("##########################################");
