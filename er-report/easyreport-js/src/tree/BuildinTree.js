...
export default class BuildinTree extends BaseTree{
    ...
    _buildDatasource(){
        ...
        const rootSpan=$(`
            <span id="${this.id}">
                ...
                <i class="easyreport easyreport-shareconnection"></i> <a href='###' class="ds_name">${this.name}</a>
            </span>`);
    ...
    addField(dataset,fields,field,ul){
        ...
        const li=$(`<li></li>`);
        const spanId=uuid.v1();
        const span=$(`<span id="${spanId}" title="${window.i18n.tree.doubleClick}"><i class="easyreport easyreport-property"></i> <a href='###'>${field.name}</a></span>`);
        li.append(span);
