...
export default class DatasetValueEditor extends BaseValueEditor{
    ...
    _init(){
        ...
        const modeGroup=$(`<div class="form-group" style="margin-left: 0;margin-top: 5px;margin-bottom: 5px;"><label style="margin-right:6px;">绑定方式：</label></div>`);
        this.propertyModeRadio=$(`<label class="checkbox-inline" style="padding-left: 2px;margin-right: 8px;"><input type="radio" name="__property_mode" value="name" checked>字段名</label>`);
        this.indexModeRadio=$(`<label class="checkbox-inline" style="padding-left: 2px;"><input type="radio" name="__property_mode" value="index">序号</label>`);
        modeGroup.append(this.propertyModeRadio);
        modeGroup.append(this.indexModeRadio);
        dsContainer.append(modeGroup);

        const propertyGroup=$(`<div class="form-group" style="margin-left: 0;margin-top: 5px;margin-bottom: 5px;display:flex;align-items:center;gap:6px;"></div>`);
        const propertyLabel=$(`<label style="margin:0;flex:0 0 auto;">${window.i18n.property.dataset.property}</label>`);
        this.propertySelect=$(`<select class="form-control" style="display: inline-block;flex:1 1 auto;min-width:240px;padding: 2px;font-size: 12px;height: 25px;vertical-align: middle;"></select>`);
        this.indexInput=$(`<input type="number" min="1" class="form-control" style="display: none;flex:1 1 auto;min-width:240px;padding: 2px;font-size: 12px;height: 25px;vertical-align: middle;" placeholder="输入列序号，从1开始">`);
        propertyGroup.append(propertyLabel);
        propertyGroup.append(this.propertySelect);
        propertyGroup.append(this.indexInput);
        dsContainer.append(propertyGroup);

        this.propertyModeRadio.children('input').click(function(){
            _this._switchMode(false);
        });
        this.indexModeRadio.children('input').click(function(){
        ...
        this.indexInput.change(function(){
            const value=$(this).val();
            _this._setIndex(value);
        });
    ...
    buildLineHeight(){
        const _this=this;
        const group=$(`<div class="form-group" style="margin-left: 0;margin-top: 5px;margin-bottom: 5px;"><label>${window.i18n.property.dataset.lineHeight}</label></div>`);
    ...
    _setIndex(index){
        ...
        for(let i=this.rowIndex;i<=this.row2Index;i++){
            for(let j=this.colIndex;j<=this.col2Index;j++){
                ...
                const valueType=cellDef.value.type;
                if(valueType==='dataset'){
                    cellDef.value.index=isNaN(idx)?null:idx;
                    cellDef.value.property=''; // 避免旧字段名干扰
                }
            }
        }
        this._updateTableData();
    ...
    _switchMode(useIndex){
        const showIndex=useIndex===true;
        if(this.cellDef && this.cellDef.value){
            this.cellDef.value.useIndex=showIndex;
        }
        if(showIndex){
            this.indexInput.css('display','inline-block');
            this.propertySelect.hide();
            const idx=this.indexInput.val();
            this._setIndex(idx);
        }else{
            this.indexInput.css('display','none');
            this.propertySelect.show();
            this._setProperty(this.propertySelect.val());
        }
    }
        ...
        for(let i=this.rowIndex;i<=this.row2Index;i++) {
            for (let j = this.colIndex; j <= this.col2Index; j++) {
                ...
                if(valueType==='simple'){
                    ...
                    const propText=value.useIndex ? ("#"+(value.index||'')) : (value.property||'');
                    data=value.datasetName+"."+value.aggregate+"("+propText+")";
    ...
    show(cellDef,rowIndex,colIndex,row2Index,col2Index){
        ...
        if(value.index){
            this.indexInput.val(value.index);
        }
        if(value.useIndex){
            this.indexModeRadio.children('input').prop('checked',true);
            this._switchMode(true);
        }else{
            this.propertyModeRadio.children('input').prop('checked',true);
