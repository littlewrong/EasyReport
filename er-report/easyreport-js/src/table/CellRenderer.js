...
export function afterRenderer(td,row,col,prop,value,cellProperties){
    ...
    if(valueType==='dataset'){
        ...
        const imagePath=window._server+'/res/easyreport-asserts/icons/image.svg';
        ...
        let imagePath=window._server+'/res/easyreport-asserts/icons/qrcode.svg';
        tip=`${window.i18n.table.render.qrcode}`;
        if(cellValue.category==='barcode'){
            tip=`${window.i18n.table.render.barcode}`;
            imagePath=window._server+'/res/easyreport-asserts/icons/barcode.svg';
        }
    ...
    if(cellDef.expand === 'Down'){
        let url=window._server+"/res/easyreport-asserts/icons/expr-expand-down.svg";
        if(valueType==='dataset'){
            url=window._server+"/res/easyreport-asserts/icons/expand-down.svg";
        }
        $td.prepend(`<image src="${url}"></image>`);
    }else if(cellDef.expand === 'Right'){
        let url=window._server+"/res/easyreport-asserts/icons/expr-expand-right.svg";
        if(valueType==='dataset'){
            url=window._server+"/res/easyreport-asserts/icons/expand-right.svg";
        }
        $td.prepend(`<image src="${url}" style="display: block;"></image>`);
    }else{
        if(valueType==='dataset'){
            let url=window._server+"/res/easyreport-asserts/icons/property.svg";
            $td.prepend(`<image src="${url}" style="display: inline-block;"></image>`);
        }else if(valueType==='expression'){
            let url=window._server+"/res/easyreport-asserts/icons/expression.svg";
            $td.prepend(`<image src="${url}" style="display: inline-block;"></image>`);
        }
    }
    if(cellStyle.align){
