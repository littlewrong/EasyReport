...
export default class ImportDialog{
    ...
    initBody(body,footer){
        ...
        body.append(form);
        form.append(`<div style="margin-bottom: 5px;color: #929191;">${window.i18n.dialog.import.desc}</div>`);
        const fileGroup=$(`<div class="form-group"><label>${window.i18n.dialog.import.file}</label></div>`);
        form.append(fileGroup);
        const file=$(`<input name="_excel_file" class="form-control" type="file" style="height:40px">`);
        ...
        const iframe=$(`<iframe height="0px" width="0px" style="border:none;" src="" name="_import_excel_frame"></iframe>`);
        body.append(iframe);
