...
export default class SqlDatasetDialog{
    constructor(db,data){
        ...
        this.dialog=$(`<div class="modal fade" role="dialog" aria-hidden="true" style="z-index: 10000;overflow: auto">
            <div class="modal-dialog modal-lg" style="width: 95vw;max-width: 1600px;margin:10px auto;">
        ...
        const container=$(`<div style="display:flex;align-items:flex-start;gap:15px;"></div>`);
        body.append(container);
        const leftContainer=$(`<div style="flex:0 0 420px;height: 80vh;overflow: auto;"></div>`);
        const rightContainer=$(`<div style="flex:1;min-width:0;"></div>`);
    ...
    initTables(container){
        const searchGroup=$(`<div class="form-group" style="margin-bottom: 5px;display:flex;align-items:center;gap:2px;"></div>`);
        container.append(searchGroup);
        const searchEdior=$(`<input class="form-control" placeholder="${window.i18n.dialog.sql.search}" style="flex:1;width: 150px;">`);
        searchGroup.append(searchEdior);
        const searchButton=$(`<button class="btn btn-default" style="flex:0 0 auto;"><i class="glyphicon glyphicon-search"></i></button>`);
        ...
        const table=$(`<table class="table table-bordered" style="font-size: 12px"><thead><tr style="height: 30px;background: #fafafa"><td style="width: 300px;vertical-align: middle">${window.i18n.dialog.sql.tableName}</td><td style="width: 60px;vertical-align: middle">${window.i18n.dialog.sql.type}</td></tr></thead></table>`);
    ...
    initSqlEditor(body){
        ...
        this.nameEditor=$(`<input type="text" class="form-control" style="font-size: 13px;width:100%;display: inline-block">`);
        ...
        this.sqlEditor=$(`<textarea placeholder="select username,dept_id from employee where dept_id=:deptId" class="form-control" rows="14" cols="30" style="width: 100%;"></textarea>`);
    ...
    show(onSave,params){
        ...
        setTimeout(()=>{
            if(!this.codeMirror){
                ...
                this.codeMirror.setSize('100%','320px');
            }
