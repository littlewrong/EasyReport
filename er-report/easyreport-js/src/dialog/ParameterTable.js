...
export default class ParameterTable{
    constructor(container,data){
        this.data=data;
        const addButton=$(`<button type="button" class="btn btn-info" style="margin-bottom: 5px;">${window.i18n.dialog.paramTable.addParam}</button>`);
        container.append(addButton);
        const _this=this;
        const table=$(`<table class="table table-bordered" style="margin-bottom: 0;table-layout: fixed;width: 100%;">
            <thead>
                <tr style="background: #f4f4f4;height: 30px;"><td style="vertical-align: middle;width: 150px;word-break: break-all;">${window.i18n.dialog.paramTable.paramName}</td><td style="vertical-align: middle;width: 100px;word-break: break-all;">${window.i18n.dialog.paramTable.paramDatatype}</td><td style="vertical-align: middle;word-break: break-all;">${window.i18n.dialog.paramTable.defaultValue}</td><td style="vertical-align: middle;width: 80px;">${window.i18n.dialog.paramTable.operator}</td></tr>
            </thead>
        </table>`);
        ...
        addButton.click(function(){
            _this.parameterDialog.show(function(name,type,defaultValue){
                ...
                const nameTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 150px;">${name}</td>`);
                const typeTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 100px;">${type}</td>`);
                const defaultValueTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 300px;">${defaultValue}</td>`);
                newTr.append(nameTd);
                newTr.append(typeTd);
    ...
    _buildParameters(){
        ...
        for(let param of this.data){
            ...
            const nameTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 150px;">${param.name}</td>`);
            const typeTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 100px;">${param.type}</td>`);
            const defaultValueTd=$(`<td style="vertical-align: middle;word-break: break-all;max-width: 300px;">${param.defaultValue}</td>`);
            tr.append(nameTd);
            tr.append(typeTd);
