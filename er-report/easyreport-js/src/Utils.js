...
export function showLoading(){
    ...
    const cover=$(`<div class="easyreport-loading-cover" style="position: absolute;left: 0px;top: 0px;width:${w*2}px;height:${h*2}px;z-index: 1199;background:rgba(222,222,222,.5)"></div>`);
    $(document.body).append(cover);
    const loading=$(`<div class="easyreport-loading" style="text-align: center;position: absolute;z-index: 1120;left: ${w-35}px;top: ${h-35}px;"><img src="${url}">
    <div style="margin-top: 5px">打印数据加载中...</div></div>`);
    $(document.body).append(loading);
};

export function hideLoading(){
    $('.easyreport-loading-cover').remove();
    $('.easyreport-loading').remove();
};
        ...
        for(let j=0;j<countCols;j++){
            ...
            if(cellDef){
                ...
                if(valueType==='dataset'){
                    ...
                    let prop=value.useIndex ? ("#"+(value.index || '')) : (value.property || '');
                    if(prop.length>13){
...
export function tableToXml(context){
    ...
    for(let i=0;i<countRows;i++){
        ...
        for(let header of rowHeaders){
            if(header.rowNumber===i){
                band=header.band;
                pageBreak=header.pageBreak || false;
                break;
            }
        }
        let rowXml=`<row row-number="${i+1}" height="${height}"`;
        if(band){
            rowXml+=` band="${band}"`;
        }
        if(pageBreak){
            rowXml+=` page-break="true"`;
        }
        rowXml+=`/>`;
        rowsXml+=rowXml;
    }
    ...
    for(let i=0;i<countRows;i++){
        for(let j=0;j<countCols;j++){
            ...
            if(value.type==='dataset'){
                ...
                if(!msg){
                    const usingIndex=value.useIndex===true || value.useIndex==='true' || (value.index!==null && value.index!==undefined && value.index!=='');
                    if(usingIndex){
                        if(value.index===null || value.index===undefined || value.index===''){
                            msg=`${cellName}单元格序号不能为空！`;
                        }
                ...
                cellXml+=`<dataset-value dataset-name="${encode(value.datasetName)}" aggregate="${value.aggregate}" property="${encode(property)}" order="${value.order}" mapping-type="${mappingType}"`;
                if(value.useIndex){
                    cellXml+=` use-index="true"`;
                    if(value.index){
                        cellXml+=` index="${value.index}"`;
                    }
                }
    ...
    for(let datasource of datasources){
        ...
        if(type==='jdbc'){
            ...
            for(let dataset of datasource.datasets){
                ...
                if(dataset.url){
                    ds+=` url="${encode(dataset.url)}"`;
                }
                if(dataset.method){
                    ds+=` method="${dataset.method}"`;
                }
                if(dataset.dataPath){
                    ds+=` dataPath="${encode(dataset.dataPath)}"`;
                }
                ds+=`>`;
                if(dataset.requestBody){
                    ds+=`<requestBody><![CDATA[${dataset.requestBody}]]></requestBody>`;
                }
                ...
                if(dataset.fields && dataset.fields.length>0){
                    for(let field of dataset.fields){
                        ds+=`<field name="${encode(field.name)}"/>`;
                    }
                }
                ds+=`</dataset>`;
            }
    ...
    const apiPageParamName=paper.apiPageParamName || 'page';
    const apiPageSizeParamName=paper.apiPageSizeParamName || 'page_size';
    const apiDefaultPageSize=paper.apiDefaultPageSize!==null && paper.apiDefaultPageSize!==undefined ? paper.apiDefaultPageSize : '';
    const apiMaxPageSize=paper.apiMaxPageSize!==null && paper.apiMaxPageSize!==undefined ? paper.apiMaxPageSize : '';
    if(paper.apiPagingEnabled){
        xml+=` api-paging-enabled="${paper.apiPagingEnabled}"`;
    }
    xml+=` api-page-param="${apiPageParamName}" api-page-size-param="${apiPageSizeParamName}"`;
    if(apiDefaultPageSize!==''){
        xml+=` api-default-page-size="${apiDefaultPageSize}"`;
    }
    if(apiMaxPageSize!==''){
        xml+=` api-max-page-size="${apiMaxPageSize}"`;
    }
    const apiTotalCountPath=paper.apiTotalCountPath || 'data.total_count';
    xml+=` api-total-count-path="${apiTotalCountPath}"`;
    const apiPageSize=paper.apiPageSize!==null && paper.apiPageSize!==undefined ? paper.apiPageSize : 100;
    xml+=` api-page-size="${apiPageSize}"`;
    const apiStartFieldName=paper.apiStartFieldName || 'start';
    xml+=` api-start-field="${apiStartFieldName}"`;
    const apiEndFieldName=paper.apiEndFieldName || 'end';
    xml+=` api-end-field="${apiEndFieldName}"`;
    if(paper.apiFieldMapping){
        xml+=` api-field-mapping="${paper.apiFieldMapping}"`;
    }
    if(paper.apiDatasetName){
        xml+=` api-dataset-name="${paper.apiDatasetName}"`;
    }
    if(paper.apiSummaryEnabled){
        xml+=` api-summary-enabled="${paper.apiSummaryEnabled}"`;
    }
    if(paper.apiSummaryDatasetName){
        xml+=` api-summary-dataset-name="${paper.apiSummaryDatasetName}"`;
    }
    if(paper.apiSummaryFieldMapping){
        xml+=` api-summary-field-mapping="${paper.apiSummaryFieldMapping}"`;
    }
    if(paper.apiSummaryLabel){
        xml+=` api-summary-label="${paper.apiSummaryLabel}"`;
    }
