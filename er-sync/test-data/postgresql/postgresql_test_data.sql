-- Generated from test-data/mysql/mysql_test_data.sql for PostgreSQL.
-- ============================================================
...
CREATE TABLE test_order (
    ...
    discount NUMERIC(5,4) DEFAULT 1.0000,
...
CREATE TABLE test_numeric (
    ...
    col_decimal_18_6 NUMERIC(18,6),
    col_decimal_38_10 NUMERIC(38,10),
    col_bit BOOLEAN,
    col_bit8 BIGINT,
    time_stamp TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id)
...
(1, 'A', 'Hello World', 'short', '这是一段中文VARCHAR(255)测试', '这是VARCHAR(2000)的长文本测试，包含特殊字符：!@#$%^&*()_+-=[]{}|;:,.<>?/', 'TinyText内容', 'TEXT类型的内容，可以存储较长的文本。This is a text column for testing.', 'MediumText：中等长度文本内容', 'LongText：超长文本内容，用于测试大文本字段的同步', 'A', 'read,write', '{"name":"test","value":123,"nested":{"key":"val"}}', CURRENT_TIMESTAMP),
(2, 'B', '', '', '', '', '', '', '', '', 'B', 'exec', '[]', CURRENT_TIMESTAMP),
(3, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'C', NULL, NULL, CURRENT_TIMESTAMP),
(4, 'Z', '  spaces  ', '带空格', '包含\t制表符的文本', '包含\n换行\n多行文本', 'Tiny', '包含emoji：测试数据', 'Medium', 'Long', 'D', 'read,write,exec', '{"arr":[1,2,3],"bool":true,"null_val":null}', CURRENT_TIMESTAMP),
(5, '中', '中文字符测试', '中文短串', '日本語テスト Korean 한국어 Mixed 混合', 'Très bien! Ñoño Ünïcödë', 'Tiny中文', '中文Text内容', '中文Medium内容', '中文Long内容，测试各种字符编码的兼容性', 'A', 'read', '{"unicode":"中文","emoji":"ok"}', CURRENT_TIMESTAMP);
...
CREATE TABLE test_datetime (
    ...
    col_datetime_3 TIMESTAMP(3),
    col_datetime_6 TIMESTAMP(6),
...
(1, '\x550E8400E29B41D4A716446655440000'::bytea, '\xDEADBEEF'::bytea, '\x48656C6C6F'::bytea, '\x48656C6C6F20576F726C64'::bytea, '\xCAFEBABE'::bytea, '\x0102030405060708090A'::bytea, CURRENT_TIMESTAMP),
(2, '\x00000000000000000000000000000000'::bytea, '\x00'::bytea, '\x00'::bytea, '\x00'::bytea, '\x00'::bytea, '\x00'::bytea, CURRENT_TIMESTAMP),
...
CREATE TABLE test_default_values (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    col_default_str VARCHAR(50) NOT NULL DEFAULT 'hello',
    col_default_int INTEGER NOT NULL DEFAULT 42,
    col_default_dec NUMERIC(8,2) NOT NULL DEFAULT 3.14,
    col_default_ts TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    col_not_null VARCHAR(100) NOT NULL,
    col_nullable VARCHAR(100),
    time_stamp TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id)
);
...
SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE 'test%' ORDER BY table_name;
